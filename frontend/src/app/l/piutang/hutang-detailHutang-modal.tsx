...
const DetailPiutangModal: React.FC<DetailHutangProps> = ({ id,onClose }) => {
  ...
  const { data = [], error, isLoading, mutate } = useSWR(`${API_URL}api/araps/${id}/`, fetcher);
  ...
  const selectedTransaction = data.transactions?.find((item: any) => String(item.id) === selectedFaktur);
  
  function formatAmount(amount: string | number) {
    const angka = typeof amount === "string" ? Math.floor(Number(amount)) : Math.floor(amount);
    ...
    <div className=" flex flex-col " >
      <DialogHeader>
        <DialogTitle className="text-sm font-bold text-gray-800 mb-2">
          Pembayaran Piutang - {data.customer_name}
        </DialogTitle>
      </DialogHeader>
      ...
      <div className="flex  gap-4 mb-3 ">
        ...
        <div className="flex flex-col gap-2 justify-between w-full">
          <div className="flex  justify-between">
            <Label htmlFor="Pemasok" className='text-xs '>Customer</Label>
            ...
            {selectedTransaction ? (
              <>
                <TableRow >
                  ...
                  <TableCell className="text-right border-l pr-3 max-w-[200px]">{`Rp${selectedTransaction.remaining_amount.toLocaleString('id-ID')}`}</TableCell>
                </TableRow>
                <TableRow>
                ...
                <TableRow >
                  ...
                  <TableCell className="text-right border-l pr-3 max-w-[200px]">{`Rp${sisaHutang.toLocaleString('id-ID')}`}</TableCell>
                </TableRow>
              </>
            ) : (
              <TableRow>
                <TableCell colSpan={2} className="text-center text-gray-400">Tidak ada detail hutang</TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <div className="flex justify-end mb-0 pb-0 ">
        <Button className="bg-blue-500  hover:bg-blue-600" onClick={onSubmit}>Bayar</Button>
      </div>
    </div>
