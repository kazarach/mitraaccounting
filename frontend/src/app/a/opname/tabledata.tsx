...
const OpnameReport = () => {
  ...
  ? `${API_URL}api/transactions/?th_type=ADJUSTMENT&start_date=${startDate}&end_date=${endDate}`
  : range
  ? `${API_URL}api/transactions/?th_type=ADJUSTMENT&range=${range}`
  : null;
  ...
  const { data: json, error, isLoading } = useSWR(
    ...
    return json.flatMap((transaction: any) =>
      transaction.items.map((item: any) => ({
        date: format(new Date(transaction.th_date), "dd/MM/yyyy"),
        faktur: transaction.th_code,
        id: transaction.id,
